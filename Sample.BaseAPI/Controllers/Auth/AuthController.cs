...
namespace Sample.BaseAPI.Controllers.Auth
{
    [ApiController]
    public abstract class AuthController : ControllerBase
    {
        ...
        [Authorize]
        public virtual async Task<AppDomainResult> ChangePassword(int userId, [FromBody] ChangePassword changePasswordModel)
        {
            AppDomainResult appDomainResult = new AppDomainResult();
            if (ModelState.IsValid)
            {
        ...
        protected async Task<string> GenerateJwtToken(UserModel user, bool isConfirmOTP = false)
        {
            ...
            //var fileAvatar = await this.userFileService.GetSingleAsync(e => !e.Deleted && e.Active 
            //&& e.UserId == user.Id 
